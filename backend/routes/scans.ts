...
router.post('/', async (req: Request, res: Response) => {
    ...
    res.status(201).json(new ApiResponse(true, 'Scan created'));
});
...
router.post('/:id/analyze', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (error) {
            ...
            return res.status(500).json(new ApiResponse(false, 'Failed to fetch scan keyframes'));
        }
        if (!data) {
            return res.status(404).json(new ApiResponse(false, 'Scan not found'));
        }
        ...
        if (keyframes.length === 0) {
            return res.status(400).json(new ApiResponse(false, 'No keyframes found for analysis'));
        }
        ...
        if (updateError) {
            ...
            return res.status(500).json(new ApiResponse(false, 'AI analysis completed but failed to save results'));
        }

        return res.json(new ApiResponse(true, 'AI analysis completed', {
            id: scanId,
