...
class ScanService: NSObject, ObservableObject {
    ...
    private func syncMapLandmarks(mapId: String) async -> Int {
        ...
        for landmark in landmarks.prefix(25) {
            if isAutoGeneratedARKitLandmark(label: landmark.label) {
                continue
            }
    ...
    private func isAutoGeneratedARKitLandmark(label: String?) -> Bool {
        guard let normalized = label?
        ...
        return normalized.hasPrefix("arkit ")
    }
